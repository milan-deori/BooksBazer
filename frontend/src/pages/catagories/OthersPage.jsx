...
const OthersPage = ({ user, setWishlist }) => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        ...
        if (response.ok) {
          const filtered = data
            .filter((book) => book.category === 'Others')
  ...
  return (
    <div className="bg-[#f1f2f4] min-h-screen w-full max-w-screen-2xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-semibold text-gray-800">Other Category</h1>
      <hr className="my-2 border-gray-600 mb-4" />

      {books.length === 0 ? (
        <div className="flex flex-col items-center justify-center mt-20 text-gray-500">
          <FaBoxOpen className="text-7xl mb-6" />
          <p className="text-lg font-semibold">No books found in Others category</p>
          <p className="mt-2 text-center max-w-sm">
            Check back later or explore different categories for more content.
          </p>
        </div>
      ) : (
