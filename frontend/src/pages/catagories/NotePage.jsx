...
const NotePage = ({ user, setWishlist }) => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        ...
        if (response.ok) {
          const filtered = data
            .filter((book) => book.category === 'Notes')
    ...
    <div className="bg-[#f1f2f4] min-h-screen w-full max-w-screen-2xl mx-auto px-4 py-6">
      ...
      {books.length === 0 ? (
        <div className="flex flex-col items-center justify-center mt-20 text-gray-500">
          <FaRegStickyNote className="text-7xl mb-6" />
          <p className="text-lg font-semibold">No Notes Found</p>
          <p className="mt-2 text-center max-w-sm">
            Sorry, we couldn’t find any notes at the moment. Please check back later or explore other categories.
          </p>
        </div>
      ) : (
