...
import { FaHeart, FaBookOpen } from 'react-icons/fa'; // Added FaBookOpen for default icon
import { Link, useNavigate } from 'react-router-dom';
import { toast } from 'react-toastify';
import axios from 'axios';

const BioPage = ({ user, setWishlist }) => {
  ...
  return (
    <div className="bg-[#f1f2f4] min-h-screen w-full max-w-screen-2xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-semibold text-gray-800">Biography Books</h1>
      <hr className="my-2 border-gray-600 mb-4" />

      {books.length === 0 ? (
        <div className="flex flex-col items-center justify-center mt-20 text-gray-500">
          <FaBookOpen className="text-7xl mb-6" />
          <p className="text-lg font-semibold">No Biography Books Found</p>
          <p className="mt-2 text-center max-w-sm">
            Sorry, we couldn't find any biography books at the moment. Please check back later or try a different category.
          </p>
        </div>
      ) : (
